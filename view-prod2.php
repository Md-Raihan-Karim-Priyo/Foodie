...
<body class="" style="background-color: silver;">
  ...
  <header>
    ...
    <h1 class="mt-2 text-center text-light bg-dark">View Dishes Panel</h1>
  ...
  <main>
    ...
    <h1 style="text-align: center;">View  Dishes</h1>
    <hr style="border: 5px solid black; border-bottom: none;">
    </div>
    <!-- Tables starts here -->
    <?php 
        ...
        if($category=="all"){
            $sql1="select category_name from prod_category";
            $result1=mysqli_query($conn,$sql1) or die("query failed");
             if(mysqli_num_rows($result1)>0){
                while($row1=mysqli_fetch_assoc($result1)){
                    $category1=$row1['category_name'];
                    $sql ="select * from {$category1}";
                    $result=mysqli_query($conn,$sql) or die("query failed");
                    if(mysqli_num_rows($result)>0){
                    echo "<div class=''>";
                    echo "<div class='text-center'><h3 class='bg-success text-white'>";echo $category1; echo"</h3></div>";
                    echo" <hr style='border: 5px solid black; border-bottom: none;''>";
                    echo "<table class='table table-bordered p-3 text-center table-hover'>";
                    ...
                    while($row=mysqli_fetch_assoc($result)){
                        ...
                        echo"<td>";echo $row['prod_price']. " ".$row['prod_unit'];echo"</td>";
                        echo"<td>";echo $row['prod_img'];echo"</td>";
                        echo"</tr>";
                    }
                        echo "</tbody> ";
            ...
            if(mysqli_num_rows($result)>0){
                ?>
  <?php  echo "<div class=''>";
      echo "<div class='text-center'><h3>";echo $category; echo"</h3></div>";
     echo" <hr style='border: 5px solid black; border-bottom: none;''>";
   echo "<table class='table table-bordered p-3 text-center table-hover'>";
       echo" <thead style='font-weight: bolder;'>";
        echo "<tr>";
  ...
  <div class="text-center"><a  href="view-prod1.php" class="btn btn-success mb-3">Go back</a></div>
