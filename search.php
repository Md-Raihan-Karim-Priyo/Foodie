...
<body style="background-color: #E5E3E0">
    ...
    <div class="MyCardContainer mt-5 pb-3">
    <div class="title text-center mb-3">
        <hr style="border: 5px solid red; border-bottom: none;">
          <h3 class="font-weight-bolde text-light">Search: <?php echo $_POST['search_term']?> </h3>
          <hr style="border: 5px solid red; border-bottom: none;">
        </div>
        <div class="row">
    <?php 
             ...
             if(mysqli_num_rows($result)>0){
               ...
               while($row = mysqli_fetch_assoc($result)){
                   $sql1="SELECT * FROM {$row['category_name']} WHERE prod_name like '%{$search_term}%' ";
    ...
    ?>
                    ...
                    <div class="col-12 col-md-12 col-lg-4 mb-3">
                        ...
                        <?php echo '<img class="card-img-top"
                            src="images/'.$row1['prod_img'].'" style="max-height: 12rem;min-height:12rem; max-width: 18rem;"
                            alt="product image">'?>
                        <div class="card-body">
                            <h5 class="card-title"><?php echo $row1['prod_name'];  ?></h5>
                            <p class="card-text"><?php  echo $row1['prod_price']." ".$row1['prod_unit']; ?></p>
                            <form action="manage_cart.php" method="POST">
                            <button type="submit" name="Add_To_Cart" class="btn btn-dark">Add to My Cart</button>
                            <input type="hidden" name="Item_Name" value="<?php echo $row1['prod_name']?>" >
                            <input type="hidden" name="Price" value="<?php echo $row1['prod_price']?>" >
                            </form>
                        </div>
                        </div>
                    </div>
